...
-- Create or Replace Procedure to Add Return Records
...
BEGIN
    ------------------------------------------------------------------
    -- Step 1: Check if the Issued ID exists and fetch book details
    ------------------------------------------------------------------
    SELECT
        issued_book_isbn,
        issued_book_name
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'Issued ID % does not exist.', p_issued_id;
    END IF;

    ------------------------------------------------------------------
    -- Step 2: Check if the book has already been returned
    ------------------------------------------------------------------
    IF EXISTS (
        SELECT 1
        FROM return_status
        ...
        RAISE EXCEPTION 'Book with Issued ID % has already been returned.', p_issued_id;
    END IF;
    ...
    ------------------------------------------------------------------
    -- Step 4: Update book status to Available
    ------------------------------------------------------------------
    UPDATE books
    SET status = 'yes'
    WHERE isbn = v_isbn;
    ...
    ------------------------------------------------------------------
    RAISE NOTICE 'Thank you! Book "%" has been successfully returned.', v_book_name;
...
SELECT
    ...
    m.member_name,
    b.book_title,
    COUNT(i.issued_id) AS damaged_books_issued
FROM members m
